...
driver.find_element_by_xpath('/html/body/header/table/tbody/tr/td[3]/form/table/tbody/tr/td[1]/div[1]/div/input[1]').send_keys(rechercheEbay)
time.sleep(0.5)
driver.find_element_by_xpath('/html/body/header/table/tbody/tr/td[3]/form/table/tbody/tr/td[3]/input').click()
time.sleep(2)
try:
...
nomDossier = str(input("Comment souhaitez-vous nommer votre dossier pour stocker les ressources : \n"))
...
incrementation = 0
myfile = open("./" + nomDossier + "/" + str(input("Comment voulez vous nommer votre fichier ? ")) + ".txt", 'w')
for i in listDesResultats:
    try:
        img = i.find_element_by_tag_name('img')
        src = img.get_attribute('src')
        print(img.get_attribute('src'))
        urllib.request.urlretrieve(str(src), "./" + nomDossier + "/Fichier_image_"+ rechercheEbay + str(incrementation)+ ".png")
        myfile.writelines("Image associée : Fichier_image_"+ rechercheEbay + str(incrementation)+ ".png\n")
